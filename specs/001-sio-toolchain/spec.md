...
As a Shenzhen I/O player, I want to run my assembled program in a headless simulator that reflects the MCU timing model so I can debug behavior (including sleeps and message passing) and inspect traces without launching the game.
...
3. **Given** a program that communicates across MCUs (message passing), **When** it is simulated, **Then** message send/receive behavior is reflected in the outputs and trace deterministically.
...
- **FR-007**: The simulator MUST support core MCU concepts needed for typical Shenzhen I/O solutions: registers, ports, and inter-node/message passing behavior.
...
- **NFR-004**: The project MUST be open-source and licensed under MIT.
